...
# 全キーワード集合（単語判定用）
_ALL_KEYWORDS = set()
for mapping in [_ERA_KEYWORDS, _ORIGIN_KEYWORDS, _STYLE_KEYWORDS, _RENOWN_KEYWORDS, _CATEGORY_KEYWORDS]:
...
class QueryVectorizer:
    ...
    def vectorize(cls, query: str) -> Tuple[list[float], Optional[str], Optional[str]]:
        ...
        elif len(query_lower.split()) == 1 and not any(k in query_lower for k in _ALL_KEYWORDS):
