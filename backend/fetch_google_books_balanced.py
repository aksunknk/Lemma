...
KEYWORDS = ['小説', 'ビジネス', '自己啓発', '歴史', '科学', 'エッセイ', '専門書', '哲学']
...
def fetch_google_books():
    ...
    for kw in KEYWORDS:
        ...
        for year in YEARS:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] Keyword: {kw} ({year}) | Total: {total_added} / {TARGET_COUNT}")
            
            for start_index in range(0, 120, 40): # ジャンル・年あたり120件
                if total_added >= TARGET_COUNT: break
                
                params = {
                    'q': f'{kw} after:{year-1} before:{year+1}',
                    'langRestrict': 'ja',
                ...
                try:
                    ...
                    for item in items:
                        ...
                        if isbn:
                            # ベクトル計算
                            era, origin, style, renown = calculate_heuristics(title, author, publisher, description, pubdate)
